...
describe('测试Promise', function () {

  it('测试Promise的创建', function () {
    let p1 = new Promise(() => {})
    let p2 = new Promise(() => {})
    expect(p1).to.have.all.keys('_state', '_tasks', '_value', 'fn')
    expect(p1._state).to.equal(p2._state)
    expect(p1._tasks).to.deep.equal([])
    expect(p1._value).to.equal(null)
  })

  it('测试then的添加', function () {
  ...
  it('测试then的添加, resolve回调', function () {
    let p = new Promise(function (resolve, reject) {
      resolve(1)
    }).then(function (res) {
  ...
  it('测试then的添加, reject回调', function () {
    let p = new Promise(function (resolve, reject) {
      reject(1)
    }).then(function (res) {
    }, function (err) {
      expect(1).to.equal(err)
    })
  })

  it('测试then的添加, 多次链式调用', function () {
    let p = new Promise(function (resolve, reject) {
      resolve(1)
    }).then(function (res) {
      expect(1).to.equal(res)
  ...
  it('测试catch的添加', function () {
    let p = new Promise(function (resolve, reject) {
    }).catch(function () {
    })
  })

  it('测试catch的添加, reject', function () {
    let p = new Promise(function (resolve, reject) {
      reject(1)
    }).catch(function (err) {
      expect(1).to.equal(err)
  ...
  it('测试Promise.race', function () {
    ...
    Promise.race([p1(), p2(), p3()]).then((res) => {
      expect(1).to.deep.equal(res)
    })
  })

  it('测试Promise.reject', function () {
    Promise.reject(3).catch((err) => {
      expect(3).to.equal(err)
    })
  })

  it('测试Promise.resolve', function () {
